...
Daemon flags worth knowing: `--socket`, `--root`, `--changelog-capacity` (ring size, default 4096), `--search-cache-capacity` (LRU size, default 64; 0 disables), `--no-prewarm` (skip startup page-cache walk — useful in tests/benchmarks).
...
- **`crates/daemon`** — the `mcp-cli-daemon` binary. Owns the hot state. Modules:
  ...
  - `handlers.rs` — per-method logic: `fs.read` (mmap via `memmap2`), `git.status` (libgit2), `search.grep` (grep-searcher + grep-regex), `fs.scan` (ignore::WalkBuilder), `fs.snapshot`/`fs.changes` (read from `ChangeLog`).
  ...
  - `watcher.rs` — notify-rs (`FSEvent` on macOS / inotify on Linux) feeding the `ChangeLog`. Gitignore-aware, hard-excludes `.git/`.
...
See `doc/roadmap.md` and `doc/todo.md`. M0 (skeleton) and M1 (incremental sync) are done. M2 is indexing (tree-sitter symbol index, persistent cache keyed by `ChangeLog` version). M3 is pluggable language backends (rust-analyzer, clangd) behind a `LanguageBackend` trait. New functionality should slot into one of those milestones rather than grow ad-hoc handlers.
